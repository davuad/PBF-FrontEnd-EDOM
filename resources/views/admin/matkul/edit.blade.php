...
<body class="font-poppins text-[#0A090B]">
    <section id="content" class="flex">
        ...
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <form method="POST" action="{{ route('admin.matkul.update', $matkul['id_matkul'])  }}" class="flex flex-col gap-[30px] w-[500px]">
              @csrf
              @method('PUT')
              ...
              <div class="flex flex-col gap-[10px]">
                ...
                <div class="flex items-center h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE]">
                  <input type="text" name="nama_matkul" placeholder="Contoh: Struktur Data" value="{{ old('nama_matkul', $matkul['nama_matkul']) }}" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" required>
                </div>
                ...
                <div class="flex items-center h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE]">
                  <select name="id_prodi" id="prodiSelect" class="font-semibold w-full outline-none bg-transparent" required>
                    ...
                    @foreach($prodiList as $prodi)
                      <option value="{{ $prodi['id_prodi'] }}" {{ $matkul['id_prodi'] == $prodi['id_prodi'] ? 'selected' : '' }}>
                        {{ $prodi['nama_prodi'] }}
                      </option>
                    @endforeach
                  </select>
              ...
              <div class="flex flex-col gap-[10px]">
                ...
                <div class="flex items-center h-[52px] p-[14px_16px] rounded-full border border-[#EEEEEE]">
                  <input type="number" name="sks" placeholder="Contoh: 3" value="{{ old('sks', $matkul['sks']) }}" class="font-semibold placeholder:text-[#7F8190] placeholder:font-normal w-full outline-none" required>
                </div>
